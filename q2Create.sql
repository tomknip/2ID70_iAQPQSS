...
CREATE MATERIALIZED VIEW finishedStudNoFails(studentId, studentregistrationid, sumECTS, GPA) as SELECT P.studentId, P.studentregistrationid, P.sumECTS, P.GPA FROM courseRegistrations CR, finishedStud FS WHERE CR.StudentId = FS.StudentId GROUP BY P.studentId, P.studentregistrationid, P.sumECTS, P.GPA HAVING min(CR.grade) >= 5;
